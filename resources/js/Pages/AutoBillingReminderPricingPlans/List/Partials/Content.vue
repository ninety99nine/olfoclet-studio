                ...
                <el-popover :width="400">
                    ...
                    <template #default>
                        <span v-if="projectPayload.can_auto_bill" class="break-normal">
                            Turning off "Auto Billing" from the project settings means that every pricing plan won't be able to auto bill even if the pricing plan "Auto Billing" option is turned on. Additionally, subscribers will not be sent auto billing reminders via SMS.
                            <hr class="my-4">
                        ...
                        <span v-else class="break-normal">
                            Turning on "Auto Billing" from the project settings means that every pricing plan will be able to auto bill as long as the pricing plan "Auto Billing" option is turned on. Additionally, subscribers will be sent auto billing reminders via SMS.
                        </span>
                    </template>
                </el-popover>
                    ...
                    <div class="shadow border-b border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="pricingPlan in autoBillingReminderPricingPlansPayload.data" :key="pricingPlan.id">
                                    <!-- Name -->
                                    ...
                                    <!-- Status -->
                                    <td class="px-6 py-3">
                                        <AutoBillingReminderStatusBadge :autoBillingReminderJobBatch="getLatestPricingPlanBatchJob(pricingPlan)"></AutoBillingReminderStatusBadge>
                                    </td>
                                    <!-- Sprints -->
                                    <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-500 text-center">
                                        <div class="text-sm text-gray-900">{{ pricingPlan.auto_billing_reminder_job_batches_count }}</div>
                                    </td>
                                    <!-- Total -->
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="#" @click.prevent="$inertia.get(route('show.auto.billing.pricing.plan.reminder.job.batches', { project: route().params.project, pricing_plan: pricingPlan.id }))" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                    </td>
                                </tr>

                                <tr v-if="autoBillingReminderPricingPlansPayload.data.length == 0">
                                    <!-- Content -->
...
<script>
    ...
    export default defineComponent({
        components: {
            Pagination, PricingPlanCanAutoBillBadge, AutoBillingReminderStatusBadge
        },
        props: {
            projectPayload: Object,
            autoBillingReminderPricingPlansPayload: Object,
        },
        data() {
            return {
        ...
        methods: {
            ...
            {
                if( pricingPlan.latest_auto_billing_reminder_job_batch.length ) {
