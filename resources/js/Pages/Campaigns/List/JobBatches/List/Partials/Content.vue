    ...
    <div>
        ...
        <div class="mb-6 gap-4">
            ...
            <div class="bg-gray-50 border-b pl-6 py-3 rounded-t text-gray-500 text-sm mb-6">
                ...
                <!-- Schedule Type -->
                <span>{{ campaign.schedule_type }}</span>

                <!-- Recurring Schedule -->
                <span v-if="campaign.schedule_type == 'Send Recurring'">
                    <span class="text-gray-300 mx-4">|</span>
                    <span>Every {{ campaign.recurring_duration }} {{ recurringFrequencyWord }}</span>
                    <span class="text-gray-300 mx-4">|</span>
                    <span><span class="text-gray-400">Start: </span>{{ moment(campaign.start_datetime).format('lll') }}</span>
                    <span class="text-gray-300 mx-4">|</span>
                    <span><span class="text-gray-400">End: </span>{{ moment(campaign.end_datetime).format('lll') }}</span>
                </span>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="campaignBatchJob in campaignBatchJobsPayload.data" :key="campaignBatchJob.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ campaignBatchJob.created_at == null ? '...' : moment(campaignBatchJob.created_at).format('lll') }}
                                </td>
                            </tr>
    ...
    export default defineComponent({
        ...
        computed: {
            recurringFrequencyWord(){
                if(this.campaign.recurring_duration == 1) {

                    if( this.campaign.recurring_frequency == 'Years' ){

                        return 'Year';

                    }else if( this.campaign.recurring_frequency == 'Months' ){

                        return 'Month';

                    }else if( this.campaign.recurring_frequency == 'Weeks' ){

                        return 'Week';

                    }else if( this.campaign.recurring_frequency == 'Days' ){

                        return 'Day';

                    }else if( this.campaign.recurring_frequency == 'Hours' ){

                        return 'Hour';

                    }else if( this.campaign.recurring_frequency == 'Minutes' ){

                        return 'Minute';

                    }else if( this.campaign.recurring_frequency == 'Seconds' ){
                    ...
                    //  This returns Years, Months, Weeks, Days, Hours, Minutes or Seconds
                    return this.campaign.recurring_frequency;
