    ...
    <div class="min-h-screen bg-slate-50/50 p-4 lg:p-8 font-sans antialiased text-slate-700">
        <div class="max-w-[1600px] mx-auto mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h1 class="text-2xl font-black tracking-tight text-indigo-950">SMS Messages</h1>
                ...
                <div class="flex items-center gap-6">
                    <div class="text-right">
                        ...
                        <span class="text-xl font-bold text-indigo-900 tabular-nums leading-none">
                            {{ (subscriberMessagesPayload.total ?? 0).toLocaleString() }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-[1600px] mx-auto space-y-4">
            ...
            <div v-if="selectedDateOption?.value !== 'all' || selectedFilterTags.length > 0 || selectedSortOptions.length > 0" class="flex flex-wrap items-center gap-2">
                ...
                >
                    ...
                    <button type="button" aria-label="Remove date" @click="selectedDateOption = { value: 'all', label: 'All Time' }; filters.date_from = ''; filters.date_to = ''; fetchSubscriberMessages(1)" class="ml-2 text-indigo-500 hover:text-indigo-700 font-bold leading-none">×</button>
                </span>
                <span
            ...
            <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                <Transition name="content-switch" mode="out-in">
                    <div v-if="loading" key="loading" class="py-24 px-8 flex flex-col items-center justify-center text-center">
                        ...
                        <p class="text-sm font-medium text-slate-500">Loading messages...</p>
                    </div>
                    <div v-else-if="subscriberMessagesPayload.data.length > 0" key="table-wrapper" class="overflow-x-auto">
                        ...
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-left min-w-[420px] w-[45%]">Content</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-left">Type</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-left">Status</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-left">Delivery</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr
                                    v-for="row in subscriberMessagesPayload.data"
                                    :key="row.id"
                                    class="group hover:bg-indigo-50/20 transition-colors cursor-pointer"
                                    @click="goToSubscriberMessage(row.id)"
                                >
                                    <td class="px-8 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-sm text-slate-700 min-w-0 whitespace-normal break-words leading-snug">{{ row.content || '—' }}</p>
                                        <p v-if="row.character_count != null" class="text-[10px] text-slate-400 mt-0.5">{{ row.character_count }} chars</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-800 whitespace-nowrap">{{ formatType(row.type) }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <SubscriberMessageStatusBadge :subscriber-message="row" class="scale-90 origin-left" />
                                    </td>
                                    <td class="px-6 py-4">
                                        <SubscriberMessageDeliveryStatusBadge :subscriber-message="row" class="scale-90 origin-left" />
                                    </td>
                                    ...
                                    <td class="px-8 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                            <button @click.stop="goToSubscriberMessage(row.id)" class="h-8 px-3 flex items-center gap-1.5 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-indigo-500 hover:border-indigo-200 transition-all text-xs font-bold">
                                                View
                                            </button>
                                        </div>
                                    </td>
                    ...
                    <div v-else key="empty" class="py-24 px-8 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 class="text-lg font-bold text-indigo-950 mb-1">No messages found</h3>
        ...
        <Dialog v-model:visible="filterModalVisible" header="Filters" :modal="true" :dismissableMask="true" :draggable="false" :style="{ width: '380px' }"
            ...
            <div class="pt-4 pb-6 space-y-4">
                <div class="space-y-1">
                    <label class="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1 block">Send Status</label>
                ...
                <div class="space-y-1">
                    ...
                    <Select v-model="tempFilters.type" :options="typeOptions" option-label="label" option-value="value" class="w-full" showClear />
                </div>
...
import { Search, Calendar, Filter, RefreshCw, X, ChevronLeft, ChevronRight, User, MessageSquare, ArrowDownWideNarrow, Check } from 'lucide-vue-next';
...
export default defineComponent({
    components: {
        Dialog, Select, DatePicker, Pagination, SubscriberMessageStatusBadge, SubscriberMessageDeliveryStatusBadge,
        Search, Calendar, Filter, RefreshCw, X, ChevronLeft, ChevronRight, User, MessageSquare, ArrowDownWideNarrow, Check,
    },
    props: {
    ...
    data() {
        return {
            moment,
            loading: false,
            subscriberMessagesPayload: { data: [], current_page: 1, last_page: 1, total: 0, links: [] },
            searchQuery: '',
            filters: { status: '', type: null, date_from: '', date_to: '' },
            tempFilters: { status: '', type: null },
            ...
            typeOptions: [
                ...
                { label: 'Auto billing reminder', value: 'AutoBillingReminder' },
                { label: 'Auto billing disabled', value: 'AutoBillingDisabled' },
            ],
    ...
    watch: {
        loading(val) {
            if (!val && (this.subscriberMessagesPayload?.data?.length ?? 0) > 0) this.initialLoadComplete = true;
        },
        'subscriberMessagesPayload.data': {
            handler(data) {
    ...
    computed: {
        hasActiveFilters() {
            ...
            const hasLogicFilters = this.filters.status !== '' || this.filters.type != null;
        ...
        selectedFilterTags() {
            ...
            if (this.filters.type) tags.push({ key: 'type', label: this.formatType(this.filters.type) });
            return tags;
        },
        filteredPagination() {
            const current = this.subscriberMessagesPayload.current_page;
            const last = this.subscriberMessagesPayload.last_page;
            if (last <= 1) return [];
            const pages = [];
    ...
    methods: {
        ...
        fetchSubscriberMessages(page = 1) {
            const minSpinMs = 800;
            const startedAt = Date.now();
            this.loading = true;
            const url = route('show.subscriber.messages', { project: route().params.project });
            ...
            window.axios.get(url, { params }).then(({ data }) => {
                this.subscriberMessagesPayload = data.subscriberMessagesPayload;
            }).finally(stopSpinner);
        },
        refresh() { this.fetchSubscriberMessages(this.subscriberMessagesPayload.current_page || 1); },
        clearAll() {
            this.selectedDateOption = { value: 'all', label: 'All Time' };
            this.filters = { status: '', type: null, date_from: '', date_to: '' };
            this.tempFilters = { status: '', type: null };
            this.selectedSortOptions = [];
            this.searchQuery = '';
            this.fetchSubscriberMessages(1);
        },
        changePage(page) { this.fetchSubscriberMessages(page); },
        showDateModal() { this.dateModalVisible = true; },
        showFilterModal() { this.tempFilters = { ...this.filters }; this.filterModalVisible = true; },
        hideFilterModal() { this.filters = { ...this.filters, ...this.tempFilters }; this.filterModalVisible = false; this.fetchSubscriberMessages(1); },
        ...
        applyDateOption(opt) {
            ...
            if (opt.value === 'all') { this.filters.date_from = this.filters.date_to = ''; this.dateModalVisible = false; this.fetchSubscriberMessages(1); }
                ...
                this.dateModalVisible = false; this.fetchSubscriberMessages(1);
            }
        },
        ...
        goToSubscriberMessage(id) {
            const project = route().params.project;
            if (!project || !id) return;
            router.visit(route('show.subscriber.message', { project, subscriber_message: id }));
        },
    },
    watch: {
        refetchTrigger() {
            this.fetchSubscriberMessages(this.subscriberMessagesPayload.current_page || 1);
        },
    },
    created() {
        this.debouncedSearch = debounce(() => this.fetchSubscriberMessages(1), 400);
        this.fetchSubscriberMessages(1);
    },
});
</script>
...
<style>
/* Match Subscribers page badge design: amber for warn/unsuccessful */
.p-tag.p-tag-warn,
.p-tag-warn,
.tag-amber,
.p-tag.tag-amber {
